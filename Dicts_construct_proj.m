...
low_dose = single(proj);

% load('C:\Users\Liuhanxi\Desktop\Recon_matlab\FDK_highdose.mat')
load('C:\Users\Liuhanxi\Desktop\Recon_matlab\MAT\projection\high_dose_proj_monte.mat')
% xtrue  =single(rec3D);
high_dose = single(proj);

%%
xtrue      = double(high_dose(:,201:250,:));
Rec_SART   = double(low_dose(:,201:250,:));
...
%% keep about 2000 columns 
dictA =dictA_Atom(:,1:4:end);
dictB =dictB_Atom(:,1:4:end);

%save('C:\Users\Liuhanxi\Desktop\Recon_matlab\MAT\dictionary\DFR_64_4_monte_proj','dictA','dictB')

par.b = par.b(1);
dictimg = showdict(dictA(1:par.b*par.b,:),[1 1]*par.b,round(sqrt(size(dictA,2))),round(sqrt(size(dictA,2))),'lines','highcontrast');
figure, imshow(dictimg);
dictimg = showdict(dictB(1:par.b*par.b,:),[1 1]*par.b,round(sqrt(size(dictB,2))),round(sqrt(size(dictB,2))),'lines','highcontrast');
figure, imshow(dictimg);
